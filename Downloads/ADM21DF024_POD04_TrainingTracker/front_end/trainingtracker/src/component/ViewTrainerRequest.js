...
function ViewTrainerRequest() {
    ...
    useEffect(  () => {
        var temp = sessionStorage.getItem("email")
        const string=  temp.split('@')
        var result =    fetch("https://localhost:7279/api/TrainerRaiseRequests/"+string[0]+"%40"+string[1]+"?email="+string[0]+"%40"+string[1])
        .then((response) => response.json())
        .then((data) => {
          setData(data)
          console.log(data);
...
console.log()
  const res= fetch("https://localhost:7279/api/TrainerRaiseRequests/"+id, { method: 'DELETE' })
        .then(() => this.setState({ status: 'Delete successful' }));
        window.location.reload(false)
}
  return (
    <div>
<Table  className=' ttable-striped text-white'  variant="bg-transparent" bordered size='sm' >
                ...
                <tr><th colSpan={12} className="text-white text-center">MY REQUESTS</th></tr>
                    <tr className="thead-white" >
                      ...
                      <th scope="col" className="text-white">Assigned</th>
                       <th scopr="col" className="text-white">Delete</th>
                    </tr>
                </thead>
                <tbody>
